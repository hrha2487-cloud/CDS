...
```
Quickly remove a column which don’t need while keeping all others.
```{r}
...
```
Allows to rename a column while selecting it, making the output clearer or more meaningful.
...
```
The original coffeeshop dataset is unchanged.
...
```
Group according to different types.
While Regular products generally yield higher profitability ratios, Decaf products can sometimes generate significant total profit due to volume.
